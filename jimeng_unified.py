...
class JimengUnified:
    ...
    def generate(self, prompt, model, ratio, resolution, sample_strength,
        ...
        try:
            ...
            info = f"模式: {mode} | 模型: {model} | {ratio} {resolution} | URL: {image_url}"
            
            print(f"🖼️  输出尺寸: {output_image.size[0]}x{output_image.size[1]}")
            print(f"✨ 完成！")
            print(f"{'='*60}\n")
